<?php
	...
	//Get Cart-Tiket
	$ses = $_GET['id'];
	$cart_tiket = $core->get_cart($ses); //1 itu id dan akan diganti berdasarkan session
	$cart_fetch = $cart_tiket->fetch_assoc();
...
<html>
<head>
	<title>Tickects - Daftar Cart</title>
...
<body>
	...
	<!--LIST KEBERANGKATAN-->
	<!--PERULANGAN BERCABANG BY MUCH DARMAWAN-->
	<h4 class="title-tiket-fix">Tiket Pesawat :</h4>
	<?php  
		while ($row_pesawat = $cart_tiket->fetch_assoc()){
	?>
	<div class="hoverable">
		<div class="card-fix card-custom-ket text-list-berangkat-1 inline-card">
			...
			//Dapatkan Id Rute Untuk Masing" Jenis Tiket Yang Ada
			$id_rute = $row_pesawat['id_rute'];
			$fetch = $core->get_cart_by_rute($id_rute);
			?>

			<b style="float: right; margin-right: 10px;">Added On : <i><?php echo $row_pesawat['tgl_tandai']; ?></i></b>

			<?php
			while ($row_pesawat_d = $fetch->fetch_assoc()){

			?>

				<br>
				<b>To</b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: <b><i><?php echo $row_pesawat_d['tujuan']; ?></i></b>&nbsp;&nbsp;
				<br>
				<b>Routes</b> : <b><i><?php echo $row_pesawat_d['rute_awal']; echo " - "; echo $row_pesawat_d['rute_akhir']; ?></i></b>&nbsp;&nbsp;
				
				<a href="config/c-cart.php?id=<?php echo $row_pesawat_d['id_rute']; echo "&";?>id_user=<?php echo $ses; ?>" style="float:right; margin-right: 37px; margin-top: -10px;" class="tooltipped" data-position="left" data-tooltip="Delete From Cart"><i class="fa fa-trash red-text cart-trash-hover"></i></a>
				
				<br>
				<b>Price</b> &nbsp;&nbsp;&nbsp;&nbsp;: <b><i>Rp. <?php echo $row_pesawat_d['harga']; ?></i></b>&nbsp;&nbsp;
				<a href="detail-tiket.php?id=<?php echo $row_pesawat_d['id_rute']; ?>" style="float: right; margin-right: 20px;">Detail..</a>
		</div>
		<div class="card-custom-icon card-cart-color-fix inline-card white-text card-custom-icon-fix">
			<!--UNTUK ICON PESAWAT-->
			<h6 class="tgl-fix"><b><?php echo $row_pesawat_d['tgl_keberangkatan']; ?></b></h6>
			<h6 class="status-fix"><b><?php echo $row_pesawat_d['status_keberangkatan']; ?></b></h6>
		</div>
	</div>
		<br>
		<br>
	...
	?>
		<div class="card-fix card-custom-ket-2 text-list-berangkat-1 inline-card hoverable">
			<br>
			<center><h5><b><i>Cart Kosong</i></b></h5></center>
		</div>
		<br>
		<br>
